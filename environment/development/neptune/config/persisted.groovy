...
globals << [hook : [
  onStartUp: { ctx ->
    ...
    // g.io('/data/neptune/dump.json').with(IO.reader, IO.graphson).read().iterate()
  },
  onShutDown: { ctx ->
    ctx.logger.info("Dumping once at shutdown of Gremlin Server.")

    // g.io('/data/neptune/dump.json').with(IO.writer,IO.graphson).write().iterate()
  }
] as LifeCycleHook]
